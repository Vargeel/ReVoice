...
def GatedActivationUnits(filter, gate):
    return tf.tanh(filter) * tf.sigmoid(gate)


def create_weight_variable(name, shape):
    return tf.get_variable(name, shape=shape, initializer=tf.contrib.layers.xavier_initializer_conv2d(uniform=False))
...
class WaveNetModel(object):
    ...
    def __init__(self,
                 ...
                 use_biases=False,
                 scalar_input=False,
                 initial_filter_width=32, histograms=False):
    ...
    def _create_variables(self):
        ...
        with tf.variable_scope('wavenet'):
            with tf.variable_scope('causal_layer'):
                ...
                layer['filter'] = create_weight_variable(
                    'filter',
            ...
            with tf.variable_scope('dilated_stack'):
                for i, dilation in enumerate(self.dilations):
                    with tf.variable_scope('layer{}'.format(i)):
                        current = dict()
                        current['filter'] = create_weight_variable(
                            'filter',
                            [self.filter_width,
                             self.residual_channels,
                             self.dilation_channels])
                        current['gate'] = create_weight_variable(
                            'gate',
                            [self.filter_width,
                             self.residual_channels,
                             self.dilation_channels])
                        current['dense'] = create_weight_variable(
                            'dense',
                            [1,
                             self.dilation_channels,
                             self.residual_channels])
                        current['skip'] = create_weight_variable(
                            'skip',
                            [1,
                             self.dilation_channels,
                             self.skip_channels])
            ...
            with tf.variable_scope('postprocessing'):
                current = dict()
                current['postprocess1'] = create_weight_variable(
                    'postprocess1',
                    [1, self.skip_channels, self.skip_channels])
                current['postprocess2'] = create_weight_variable(
                    'postprocess2',
